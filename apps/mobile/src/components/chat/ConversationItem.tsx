...
const getConversationName = (conversation: Conversation, currentUserId: string) => {
  ...
  const partner = conversation.participants?.find((p) => String(p.userId) !== String(currentUserId));
...
const getLastMessageTime = (conversation: Conversation) => {
  const rawTime = (conversation.lastMessage as any)?.createdAt || (conversation.lastMessage as any)?.timestamp;
  if (!rawTime) return "";
    ...
    >
      ...
      <View className="flex-1 ml-3">
        <View className="flex-row items-center justify-between">
          ...
          {lastMessageTime ? (
            <Text className="text-xs text-gray-400 ml-2">{lastMessageTime}</Text>
          ) : null}
        </View>
